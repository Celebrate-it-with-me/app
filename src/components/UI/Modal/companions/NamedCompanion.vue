...
<template>
  ...
  >
    <div
      class="flex justify-end gap-x-2 mb-4"
    >
      <fwb-button
        color="yellow"
    ...
    >
      The selected guest have {{ selectedGuest.companionQty }} companions,
    ...
    >
      ...
      >
        <p class="mb-4 text-base font-medium text-gray-700 dark:text-gray-300">
          ...
          <span class="font-extralight"> {{ selectedGuest?.firstName }}
              {{ selectedGuest?.lastName }} </span>
        </p>
        <p class="mb-4 text-base font-medium text-gray-700 dark:text-gray-300">
          <strong>Email: </strong>
          <span class="font-extralight" v-if="!selectedGuest?.email"> No Email</span>
          <span class="font-extralight" v-else> {{ selectedGuest.email }}</span>
        </p>
        <p class="mb-4 text-base font-medium text-gray-700 dark:text-gray-300">
          <strong>Phone Number: </strong>
          <span class="font-extralight" v-if="!selectedGuest?.phoneNumber"> No Phone Number</span
          ><span class="font-extralight" v-else> {{ selectedGuest.phoneNumber }}</span>
        </p>
        <p class="mb-4 text-base font-medium text-gray-700 dark:text-gray-300">
