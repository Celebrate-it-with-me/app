  ...
  >
    <CCard class="max-w-md w-full p-8 shadow-xl">
      <div class="text-center mb-8">
        ...
        <p class="text-sm text-text-light mt-2">Create your account and start planning unforgettable events.</p>
      </div>
      ...
      <div v-else class="text-center">
        ...
        <router-link
          :to="'sign-in'"
          class="inline-block text-primary font-medium hover:underline"
        >
          Go to Login
        </router-link>
      </div>
...
const handleGoogleLogin = async (response) => {
  try {
    ...
    if (result.status >= 200 && result.status < 300) {
      ...
      backendError.value = true
      backendErrorMessage.value = result.response?.data?.message ?? "Google login failed. Please try again."
    }
  } catch (e) {
    console.error('Google login error:', e)
    backendError.value = true
    backendErrorMessage.value = e.response?.data?.message ?? "Google login failed. Please try again."
  } finally {
    sending.value = false
  }
}
...
const handleFacebookLogin = async () => {
  try {
    ...
    window.FB.login(async (response) => {
      if (response.authResponse) {
        ...
        if (result.status >= 200 && result.status < 300) {
          ...
          backendError.value = true
          backendErrorMessage.value = result.response?.data?.message ?? "Facebook login failed. Please try again."
        }
      } else {
        backendError.value = true
    ...
    backendError.value = true
    backendErrorMessage.value = e.response?.data?.message ?? "Facebook login failed. Please try again."
    sending.value = false
  }
}
</script>
