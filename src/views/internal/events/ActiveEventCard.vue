...
<template>
  <CCard variant="feature" class="border-l-4 border-l-rose overflow-visible relative">
    <!-- Status Badge -->
    <div class="absolute -top-3 right-4 bg-white dark:bg-gray-800 px-3 py-1 rounded-full shadow-md border border-gray-200 dark:border-gray-700">
    ...
    <template #content>
      <p class="text-sm text-gray-600 dark:text-gray-300 line-clamp-2 mb-4">{{ activeEvent.eventDescription }}</p>

      <div class="space-y-2">
        <div class="flex items-center text-sm text-gray-600 dark:text-gray-300">
          <Calendar class="w-4 h-4 mr-2 text-rose" />
          <span>{{ formatDate(activeEvent.startDate) }} – {{ formatDate(activeEvent.endDate) }}</span>
        </div>

        <div v-if="activeEvent.eventTime" class="flex items-center text-sm text-gray-600 dark:text-gray-300">
          <Clock class="w-4 h-4 mr-2 text-rose" />
          <span>{{ activeEvent.eventTime }}</span>
        </div>

        <div v-if="activeEvent.location" class="flex items-center text-sm text-gray-600 dark:text-gray-300">
