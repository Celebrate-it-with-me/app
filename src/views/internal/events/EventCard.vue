...
<template>
  <div
    class="event-card group relative overflow-hidden"
    :class="statusBorderClass"
  >
    <!-- Card Header -->
    <div class="flex justify-between items-start mb-6">
      <div class="space-y-1">
        <h3 class="text-xl font-extrabold text-gray-900 group-hover:text-purple-600 transition-colors">
          {{ event.eventName }}
        </h3>
        <p class="text-gray-500 text-sm line-clamp-1 italic">
        ...
        >
          <div class="p-2">
            <button
              ...
              class="w-full flex items-center gap-3 px-3 py-2 text-sm rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              :class="{
                'bg-purple-50 text-purple-700': status.value === event.status
              }"
            >
              <div class="w-2 h-2 rounded-full" :class="status.colorClass"></div>
              <span class="font-medium">{{ status.label }}</span>
              <span v-if="status.value === event.status" class="ml-auto text-xs text-gray-400">Current</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="mb-6">
      <div class="flex justify-between items-end mb-2">
        ...
        <span class="text-sm font-black text-purple-600">{{ Math.round(setupProgress * 100) }}%</span>
      </div>
    ...
    <!-- Metrics Grid -->
    <div class="grid grid-cols-3 gap-2 py-4 border-t border-b border-gray-50 bg-gray-50/50 -mx-6 px-6">
      <div class="text-center border-r border-gray-100">
        <div class="text-lg font-black text-gray-900">{{ event.guestsCount || 0 }}</div>
        <div class="text-[10px] text-gray-400 font-bold uppercase">Guests</div>
      </div>
      <div class="text-center border-r border-gray-100">
        <div class="text-lg font-black text-gray-900">{{ rsvpRate }}%</div>
        <div class="text-[10px] text-gray-400 font-bold uppercase">RSVP Rate</div>
      </div>
      <div class="text-center">
        <div class="text-lg font-black text-rose-500">{{ daysUntilEvent }}</div>
        <div class="text-[10px] text-gray-400 font-bold uppercase">Days Left</div>
      </div>
    </div>
    ...
    <div class="flex gap-3 mt-6">
      ...
      <button class="bg-white border border-gray-200 px-3 py-2.5 rounded-xl hover:bg-gray-50">
...
const statusBadgeClass = computed(() => {
  ...
  return badgeMap[props.event.status] || 'bg-gray-100';
});
...
const availableStatuses = computed(() => {
  ...
  const allStatuses = [
    { value: 'draft', label: 'Draft', colorClass: 'bg-yellow-400', description: 'Work in progress' },
    { value: 'published', label: 'Published', colorClass: 'bg-green-500', description: 'Live and public' },
    { value: 'archived', label: 'Archived', colorClass: 'bg-gray-400', description: 'Completed event' },
    { value: 'canceled', label: 'Canceled', colorClass: 'bg-red-500', description: 'Event canceled' }
  ];
  ...
  return allStatuses.map(status => ({
    ...status,
    disabled: !allowedTransitions[currentStatus]?.includes(status.value) && status.value !== currentStatus
  }));
});

const rsvpRate = computed(() => {
  if (props.event?.guestsCount === 0) return 0;
...
const isAlreadyActive = computed(() => props.event.id === eventStore.activeEvent?.id);
...
<style scoped>
.event-card {
  @apply bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300;
}
.status-badge {
