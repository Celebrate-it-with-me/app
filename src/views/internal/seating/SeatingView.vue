<template>
  <div class="seating-view">
      ...
      <div class="lg:col-span-1">
                    ...
                    <div>
                      ...
                      <div class="flex items-center mt-1">
                        <span
                          ...
                          :class="{
                            'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300': guest.rsvpStatus === 'confirmed',
                            'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300': guest.rsvpStatus === 'pending',
                            'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300': guest.rsvpStatus === 'declined'
                          }"
                        >
                          {{ guest.rsvpStatus.charAt(0).toUpperCase() + guest.rsvpStatus.slice(1) }}
                        </span>
                        <span v-if="guest.isCompanion" class="ml-2 px-2 py-0.5 text-xs rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                          Companion
                        </span>
                      </div>
                    </div>
        ...
        <div v-else class="h-full flex items-center justify-center">
          <div class="text-center p-6">
            ...
            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No Table Selected</h3>
            ...
            <div class="flex justify-between items-center">
              ...
              <div class="flex space-x-2">
                <button
                  ...
                  :class="{
                    'bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-white': filterStatus === status,
                    'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400': filterStatus !== status
                  }"
                >
          ...
          <template #content>
            ...
            <div v-else class="space-y-2 max-h-[500px] overflow-y-auto pr-2">
              ...
              >
                ...
                <CButton
                  v-if="selectedTable && selectedTable.assignedGuests.length < selectedTable.capacity"
                  variant="outline"
                  size="sm"
    ...
    <!-- Edit Table Modal -->
    <CModal v-model="showEditModal" showCloseIcon>
      ...
      <form @submit.prevent="updateTable">
        <div class="space-y-4">
          ...
          <CInput
            ...
            :showError="true"
          />
          <p v-if="parseInt(editingTable.capacity) < editingTable.assignedGuests.length" class="text-sm text-red-500">
            Warning: Reducing capacity will not remove assigned guests, but you won't be able to add more.
          </p>
        </div>
      </form>
      <template #footer>
...
const isFormValid = computed(() => {
  return newTable.value.name.trim() !== '' &&
...
const filteredUnassignedGuests = computed(() => {
  ...
  if (filterStatus.value !== 'all') {
    filtered = filtered.filter(guest =>
      guest.rsvpStatus === filterStatus.value
    )
  }
...
const assignGuestToTable = (guestId) => {
  ...
  if (tableIndex !== -1) {
    tables.value[tableIndex].assignedGuests.push(guest)
