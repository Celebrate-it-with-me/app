<template>
  <div>
    ...
    <div v-if="loading" class="space-y-4">
      <div v-for="i in 3" :key="i" class="bg-white rounded-2xl p-4 border border-gray-100 animate-pulse">
    ...
    <template v-else>
      <NoSweetMemories v-if="filteredMemories.length === 0 && !props.searchQuery && !props.yearFilter" @add-first-memory="addMemory" />
      ...
      <div v-if="memories?.length > 0" class="relative">
        <!-- Timeline Line -->
        <div class="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-purple-500 via-pink-500 to-purple-500 opacity-20 hidden md:block"></div>
        ...
        >
          ...
          <div v-for="memory in filteredMemories" :key="memory.id" :class="{ 'z-50 relative': openMenuId === memory.id }">
            <CCard class="relative hover:shadow-xl transition-all duration-300 border-l-4 border-purple-500 group">
              <div class="p-4">
                <!-- Drag Handle -->
                <div class="absolute top-4 left-4 cursor-move drag-handle bg-white rounded-lg p-1.5 shadow-sm hover:shadow-md hover:bg-purple-50 transition-all z-10">
                  <GripVertical class="w-4 h-4 text-gray-400 hover:text-purple-500 transition-colors" />
                </div>
                ...
                <div class="ml-7">
                  <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="relative flex-shrink-0">
                      <div v-if="memory.imagePath" class="relative overflow-hidden rounded-xl w-full md:w-40 h-40 md:h-28 bg-gray-100">
                        <img
                ...
                <div class="flex-1 flex flex-col justify-between space-y-3">
                  ...
                  <div class="flex items-start justify-between border-b border-gray-100 pb-2">
                    <div class="flex-1">
                      ...
                      <p v-if="memory.description" class="text-sm text-gray-600 dark:text-gray-400 italic line-clamp-2">
                        "{{ memory.description }}"
                      </p>
                    </div>
                    <div v-if="memory.year" class="ml-3 relative group flex-shrink-0">
                      <span class="px-3 py-1 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full text-xs font-semibold shadow-md cursor-help inline-block">
                        {{ memory.year }}
                      </span>
                      <!-- Tooltip -->
                      <div class="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                  ...
                  <div class="flex flex-wrap items-center justify-between gap-3 py-2">
                    <div class="flex flex-wrap items-center gap-3">
                      ...
                      <!-- Reactions Breakdown -->
                      <div class="flex items-center gap-2 bg-pink-50 px-2.5 py-1 rounded-full text-xs">
                        <span class="text-sm">❤️</span>
                        <span class="text-pink-600 font-semibold">{{ getReactionCount(memory, 'love') }}</span>
                        <span class="text-gray-300">|</span>
                        <span class="text-sm">😍</span>
                        <span class="text-pink-600 font-semibold">{{ getReactionCount(memory, 'heart_eyes') }}</span>
                        <span class="text-gray-300">|</span>
                        <span class="text-sm">🥺</span>
                        <span class="text-pink-600 font-semibold">{{ getReactionCount(memory, 'pleading') }}</span>
                      </div>
                      ...
                      <div class="relative">
                        ...
                        >
                          ...
                          >
                            <!-- Toggle Visibility -->
                            <button
                              @click="toggleVisibility(memory); openMenuId = null"
                              class="w-full px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-colors"
                            >
                              <svg v-if="memory.visible" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/>
                              </svg>
                            ...
                            <!-- Duplicate -->
                            <button
                              @click="duplicateMemory(memory); openMenuId = null"
                              class="w-full px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-colors"
                            >
                            ...
                            <!-- Delete -->
                            <button
                              @click="deleteMemory(memory); openMenuId = null"
