<template>
  <div class="hero-section bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg p-4 shadow-sm border-l-4 border-pink-500">
...
import { format, parse, differenceInHours, differenceInDays } from 'date-fns'
...
// Event Data
const eventData = computed(() => eventsStore.activeEvent || {
  name: "Your Event",
...
const formattedFullDate = computed(() => {
  ...
  return format(parse(eventData.value.startDate, 'MM/dd/yyyy HH:mm', new Date()), 'EEEE, MMMM d, yyyy \'at\' h:mm a');
});
