...
<template>
  <div class="activity-section bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
    ...
    <div v-if="recentActivities.length > 0" class="space-y-3">
      ...
      >
        <div v-if="index !== 2 && recentActivities.length > 1" class="absolute left-1 top-4 bottom-[-16px] w-0.5 bg-gray-100"></div>
        <div class="w-2 h-2 rounded-full bg-purple-500 mt-1 flex-shrink-0 ring-2 ring-purple-50"></div>
        <div class="flex-1 min-w-0">
          <p class="text-xs text-gray-900 leading-relaxed truncate" v-html="activity.message"></p>
          <span class="text-gray-400 text-[10px] font-medium mt-0.5 block">{{ activity.createdAt }}</span>
        </div>
      </div>
    </div>

    <!-- Empty state -->
    <div v-else class="text-center py-4 text-gray-500 border-2 border-dashed border-gray-100 rounded-lg">
