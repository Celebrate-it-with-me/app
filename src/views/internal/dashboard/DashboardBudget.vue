...
import { Wallet, Plus, Trash2, DollarSign, AlertCircle, Loader2, ArrowUpRight } from 'lucide-vue-next'
...
const budgetItems = ref([
  ...
  { id: 3, category: 'Decoration', description: 'Flowers and table settings', amount: 1200, paid: 600 },
  ...
  >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
        <div class="relative">
          <CInput
            ...
            class="w-full pl-8"
          />
          <DollarSign class="absolute left-2 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
        </div>
        <div class="relative">
          <CInput
    ...
    <div class="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
        ...
        <div class="text-xl font-bold text-gray-800 dark:text-white">${{ totalBudget.toLocaleString() }}</div>
      </div>
      <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
    ...
    <div class="overflow-y-auto max-h-64">
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead class="bg-gray-50 dark:bg-gray-800">
          <tr>
            <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Category</th>
            <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Description</th>
            ...
            <th scope="col" class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Paid</th>
            <th scope="col" class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Actions</th>
          </tr>
        </thead>
        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
          <tr v-for="item in budgetItems" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
            <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{ item.category }}</td>
            <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">{{ item.description }}</td>
            <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200 text-right">${{ item.amount.toLocaleString() }}</td>
            <td class="px-3 py-2 whitespace-nowrap text-sm text-green-600 text-right">${{ item.paid.toLocaleString() }}</td>
            <td class="px-3 py-2 whitespace-nowrap text-right text-sm font-medium">
              <button @click="removeBudgetItem(item.id)" class="text-red-500 hover:text-red-700">
    ...
    <div class="mt-4 text-right">
      <a href="#" class="text-amber-600 hover:text-amber-800 text-sm font-medium inline-flex items-center">
        View Full Budget
