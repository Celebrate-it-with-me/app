...
const headerNav = computed(() => props.config?.sections?.headerNav ?? {})
const isEnabled = computed(() => headerNav.value?.isEnabled ?? true)
const brandText = computed(() => headerNav.value?.brandText ?? 'Mis Quinces')

const items = computed(() => {
  const base = headerNav.value?.items ?? [
    { label: 'Home', hash: '#sectionHome' },
    { label: 'Save the Date', hash: '#sectionSTD' },
    { label: 'Itinerario', hash: '#sectionItinerario' },
    { label: 'Sweet Memories', hash: '#sectionSweetMemories' },
    { label: 'RSVP', hash: '#sectionRSVP' },
    { label: 'Canciones', hash: '#sectionSong' },
    { label: 'Comentarios', hash: '#sectionComments' },
    { label: 'Contacto', hash: '#sectionContact' }
  ]

  return base.filter(i => i?.hash && i?.label)
})

const leftItems = computed(() => items.value.slice(0, 4))
const rightItems = computed(() => items.value.slice(4))
          ...
          >
            ...
            >
              <ul class="w-full space-y-2">
                <li v-for="item in items" :key="item.hash">
                  <a class="hn-mobile-link" @click="handleClick(item.hash)">
        ...
        <nav>
          <ul class="flex items-center gap-x-6">
            <li
              ...
              :key="item.hash"
              class="hn-nav-item"
              :class="{ active: currentHash === item.hash }"
            >
              <a class="hn-nav-link" :href="item.hash">{{ item.label.toUpperCase() }}</a>
            </li>
          </ul>
        </nav>
