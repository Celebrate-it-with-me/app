...
const hasLocation = computed(() => !!props.config?.location?.enabled)
  ...
  <div :style="cssVars" class="hn-theme h-full min-h-screen bg-[var(--hn-bg)] text-white">
    <transition name="fade" mode="out-in">
      ...
      <main v-else ref="mainRef">
        <HNHeaderNav :config="config" />
        <HNHeroSection class="main-section" :config="config" />
        <HNSeparatorSection :config="config">
          Hay momentos en la vida que son especiales por sí solos...
        </HNSeparatorSection>

        <HNSaveTheDate class="main-section" :config="config" />

        <HNSeparatorSection :config="config">
          Será un recuerdo para atesorar, lleno de instantes mágicos que juntos vamos a crear.
        </HNSeparatorSection>

        <HNCWMItinerario class="main-section" :config="config" />
        ...
        <HNSweetMemories class="main-section" :config="config" />
        ...
        <HNRSVP class="main-section" :config="config" />

        <HNSeparatorSection :config="config">
          La música, las memorias y ustedes harán de esta noche mágica.
        </HNSeparatorSection>

        <HNSuggestedMusic class="main-section" :config="config" />

        <HNSeparatorSection :config="config">
          Que esta fiesta sea tan inolvidable como ustedes lo son para mí.
        </HNSeparatorSection>

        <HNEventComments class="main-section" :config="config" />

        <HNSeparatorSection v-if="hasLocation" :config="config">
          Donde hay amor y alegría, el momento es perfecto.
        </HNSeparatorSection>

        <HNDressCode :config="config" />

        <HNSeparatorSection v-if="hasLocation" :config="config">
          Brillemos con estilo… ahora descubramos el escenario.
        </HNSeparatorSection>

        <HNEventLocations v-if="hasLocation" :config="config" />
        ...
        <HNSwipeLeftIcon :config="config" />
        <HNEventFooter :config="config" />
        <HNBackgroundMusic :config="config" />
      </main>
    </transition>
