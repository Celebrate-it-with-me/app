...
import { Calendar, Clock, Edit, ExternalLink, MapPin } from 'lucide-vue-next'
...
const daysUntilEvent = computed(() => {
  if (!props.activeEvent?.startDate) return 0
...
const eventStatus = computed(() => {
  if (!props.activeEvent) return { text: '', color: '' }
...
<template>
  <div class="relative">
    <!-- Active Event Badge -->
    <div
      class="absolute -top-2 -right-2 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow-lg z-20"
    >
      Active
    </div>
    ...
    <CCard
      variant="feature"
      class="border-l-4 border-l-rose hover:shadow-2xl transition-all duration-500 hover:-translate-y-1"
    >
      ...
      <template v-if="activeEvent" #title>
      ...
      <template v-if="activeEvent" #content>
        <p
          class="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-6 italic leading-relaxed"
        >
          ...
          <div class="flex items-center group">
            ...
            <div class="flex flex-col">
              ...
              <span class="text-sm font-semibold text-gray-700 dark:text-gray-200"
                ...
                {{ formatDate(activeEvent.endDate) }}</span
              >
            </div>
          </div>
