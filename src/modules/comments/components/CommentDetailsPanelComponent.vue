        ...
        <div class="mt-8 flex items-center justify-between pt-6 border-t border-gray-100">
          <div class="flex items-center gap-4">
            <button
              ...
              @click="$emit('open-public', comment)"
            >
            ...
            <button
              ...
              @click="$emit('copy-guest-link', comment)"
            >
              <Copy class="w-4 h-4" />
              Copy guest link
            </button>
...
defineProps({
  ...
  // Keep this component presentational: the parent decides the label.
  authorLabel: {
    type: String,
    default: 'Guest'
  }
...
const { getInitials, formatDateTime } = useCommentFormatters()
